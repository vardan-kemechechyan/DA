...
#ifdef LIGHT_MODE_ON
		if (_UvBasedLightFactor == 1) 
		{
			float2 uv2 = i.uv * 2.0 - 1.0;
			float c = sqrt(uv2[0] * uv2[0] + uv2[1] * uv2[1]);
			return lerp(tx * _Color, float4(1.0, 1.0, 1.0, tx.a), clamp((1.02 - c - _LightFactor) * 100.0, 0, 1));
		}
		else 
		{
			return tx.a > _LightFactor ? float4(1.0, 1.0, 1.0, tx.a) : tx * _Color;
		}
#else
		return tx;
#endif
